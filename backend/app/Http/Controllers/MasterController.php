...
class MasterController extends Controller
{
    ...
    public function destroy(Request $request,string $type,int $id):JsonResponse{$this->staffOnly($request);$record=match($type){'students'=>Student::findOrFail($id),'users'=>User::findOrFail($id),'subjects'=>Subject::findOrFail($id),default=>abort(404)};if($type==='users'&&$record->id===$request->user()->id)return response()->json(['message'=>'自分自身のアカウントは削除できません。'],422);$record->delete();return response()->json(['message'=>'削除しました。']);}
